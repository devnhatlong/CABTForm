...
export const UploadFileSoftware = () => {
    ...
    const mutation = useMutationHooks(
        (data) => {
            ...
            if (uploadedFiles.length > 0) {
                const response = uploadFileService.createSoftware({
                    departmentCode,
                    uploadedFiles
                });
                return response;
    ...
    const getAllByAdmin = async (currentPage, pageSize, filters) => {
        const response = await uploadFileService.getAllFileSoftwareByAdmin(currentPage, pageSize, filters);
        return response;
    };

    const fetchGetDetail = async (rowSelected) => {
        const response = user?.role === "admin" ? await uploadFileService.getDetailFileSoftwareByAdmin(rowSelected) : await uploadFileService.getDetail(rowSelected);
    ...
    const handleDownload = async () => {
        const response = await uploadFileService.getFileSoftware(rowSelected);

        const url = window.URL.createObjectURL(new Blob([response]));
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', 'ChromeStandaloneSetup.exe');
        ...
        <div>
            ...
            {/* {user?.role !== "admin" && (
                <div style={{display: "flex", gap: "20px", marginTop: "10px" }}>
                    <Row gutter={[16, 16]}>
                        <Col flex="1">
                            <Button icon={<UploadOutlined />} style={{display: "flex", alignItems: "center", height: "50px", borderRadius: "6px"}} onClick={() => setIsModalOpen(true)}>
                                Tải báo cáo lên	
                            </Button>
                        </Col>
                    </Row>
