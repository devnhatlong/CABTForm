...
export const DigitizedDocumentByMonth = () => {
    ...
    const mutationUpdate = useMutationHooks(
        (data) => { 
            const { id, ...rests } = data;
            const response = digitizedDocumentService.updateDigitizedDocument(id, { ...rests });
            return response;
        }
    );
    ...
    const mutationDeleted = useMutationHooks(
        (data) => { 
          ...
          const res = digitizedDocumentService.deleteDigitizedDocument(id);
    ...
    const mutationDeletedMultiple = useMutationHooks(
        (data) => { 
          const { ids } = data;
          const res = digitizedDocumentService.deleteMultipleDigitizedDocument(ids);
    ...
    const fetchGetDetailDigitizedDocument = async (rowSelected) => {
        const response = await digitizedDocumentService.getDetailDigitizedDocument(rowSelected);
    ...
    const onFinish = async () => {
        if (validateRecords()) {
            ...
            message.error('Tổng số hồ sơ phải lớn hơn hoặc bằng số hồ sơ đã số hóa');
        }
    }
    ...
    return (
        <div>
            <WrapperHeader style={{marginTop: "20px"}}>Cập nhật số liệu số hóa hàng tháng</WrapperHeader>
                ...
                <div style={{ display: "flex", gap: "20px", marginTop: "10px" }}>
                    <Row gutter={[16, 16]}> {/* Tạo hàng mới với khoảng cách giữa các cột */}
                        <Col flex="1"> {/* Cột đầu tiên */}
                            <Button type="primary" style={{ display: "flex", alignItems: "center", height: "50px", borderRadius: "6px" }} onClick={() => handleOpenModal("2025")}>
                                <strong>Nhập số liệu số hóa từ 01/01/2025 đến nay</strong>
                            </Button>
                        </Col>
                    </Row>
                </div>
            ...
            <ModalComponent form={modalForm} forceRender width={1000} title="Nhập số liệu số hóa" open={isModalOpen} onCancel={handleCancel} footer={null}>
                <Form
                    name="modalForm"
                    labelCol={{ span: 9 }}
                    wrapperCol={{ span: 20 }}
                ...
                >
                    ...
                    >
                        <InputComponent type="number" name="year" value={stateDigitizedDocument.year} onChange={(e) => handleOnChange('year', e.target.value)} min="0" onKeyDown={handleKeyPress} disabled/>
                    </Form.Item>
                    <Form.Item
                        label="Tổng số tài liệu (từ 01/01/2025 đến nay)"
                    ...
                    <Form.Item
                        label="Tổng số tài liệu đã số hóa (từ 01/01/2025 đến nay)"
