...
import administrativeProceduresForm2Service from '../../services/administrativeProceduresForm2Service';
...
export const AdministrativeProceduresForm2 = () => {
    ...
    const [stateAdministrativeProceduresForm2, setStateAdministrativeProceduresForm2] = useState({
        total_csgt_records: "",
    ...
    const mutation = useMutationHooks(
        (data) => {
            ...
            const response = administrativeProceduresForm2Service.importAdministrativeProceduresForm2({
                total_csgt_records,
                total_csgt_records_digitized,
                total_admin_records,
                total_admin_records_digitized,
    ...
    const mutationUpdate = useMutationHooks(
        (data) => { 
            const { id, ...rests } = data;
            const response = administrativeProceduresForm2Service.update(id, { ...rests });
            return response;
        }
    );
    ...
    const mutationDeleted = useMutationHooks(
        (data) => { 
          ...
          const res = administrativeProceduresForm2Service.deleteItem(id);
    ...
    const mutationDeletedMultiple = useMutationHooks(
        (data) => { 
          const { ids } = data;
          const res = administrativeProceduresForm2Service.deleteMultiple(ids);
    ...
    const handleCancel = () => {
        ...
        setStateAdministrativeProceduresForm2({
            total_csgt_records: "",
            total_csgt_records_digitized: "",
            total_admin_records: "",
            total_admin_records_digitized: "",
    ...
    const getAllDigitizedDocument = async (currentPage, pageSize, filters) => {
        const response = await administrativeProceduresForm2Service.getAll(currentPage, pageSize, filters);
        return response;
    };

    const getAllDigitizedDocumentByAdmin = async (currentPage, pageSize, filters) => {
        const response = await administrativeProceduresForm2Service.getAllByAdmin(currentPage, pageSize, filters);
        return response;
    };
    ...
    const fetchGetDetailDigitizedDocument = async (rowSelected) => {
        const response = await administrativeProceduresForm2Service.getDetail(rowSelected);
    ...
    const validateRecords = () => {
        const {
            ...
            total_immigration_records,
            total_immigration_records_digitized
        } = stateAdministrativeProceduresForm2;
        ...
        if (total_csgt_records && total_csgt_records_digitized) {
            if (parseInt(total_csgt_records, 10) < parseInt(total_csgt_records_digitized, 10)) {
                return {
                    valid: false,
                    message: 'Tổng số hồ sơ CSGT đã số hóa <= Tổng số hồ sơ CSGT'
                };
            }
        }
        ...
        if (total_admin_records && total_admin_records_digitized) {
            if (parseInt(total_admin_records, 10) < parseInt(total_admin_records_digitized, 10)) {
                return {
                    valid: false,
                    message: 'Tổng số hồ sơ hành chính đã số hóa <= Tổng số hồ sơ hành chính'
                };
            }
        }
        ...
        if (total_firefighting_records && total_firefighting_records_digitized) {
            if (parseInt(total_firefighting_records, 10) < parseInt(total_firefighting_records_digitized, 10)) {
                return {
                    valid: false,
                    message: 'Tổng số hồ sơ PCCC đã số hóa <= Tổng số hồ sơ PCCC'
                };
            }
        }
        ...
        if (total_immigration_records && total_immigration_records_digitized) {
            if (parseInt(total_immigration_records, 10) < parseInt(total_immigration_records_digitized, 10)) {
                return {
                    valid: false,
                    message: 'Tổng số hồ sơ xuất nhập cảnh đã số hóa <= Tổng số hồ sơ xuất nhập cảnh'
                };
            }
        }
    ...
    const onFinish = async () => {
        ...
        if (validationResult.valid) {
            mutation.mutate(stateAdministrativeProceduresForm2);
        } else {
    ...
    const columns = [
        ...
        {
            title: 'Tổng số hồ sơ CSGT (Đã số hóa)',
        ...
        {
            title: 'Tổng số hồ sơ QLHC (Đã số hóa)',
        ...
        {
            title: 'Tổng số hồ sơ PCCC (Đã số hóa)',
        ...
        {
            title: 'Tổng số hồ sơ XNC (Đã số hóa)',
        ...
        <div>
                    ...
                    <Row gutter={[16, 16]}> {/* Tạo hàng mới với khoảng cách giữa các cột */}
                        <Col flex="1"> {/* Cột đầu tiên */}
                            <Button type="primary" style={{ display: "flex", alignItems: "center", height: "50px", borderRadius: "6px" }} onClick={() => handleOpenModal()}>
                                <strong>Nhập số liệu giai đoạn 01/07/2022 đến thời điểm hiện tại</strong>
                            </Button>
                        </Col>
                    </Row>
                </div>
            ...
            <ModalComponent form={modalForm} forceRender width={1100} title="Nhập số liệu giai đoạn 01/07/2022 đến thời điểm hiện tại" open={isModalOpen} onCancel={handleCancel} footer={null}>
                ...
                >
                    ...
                    >
                        <InputComponent type="number" name="total_csgt_records" value={stateAdministrativeProceduresForm2.total_csgt_records} onChange={(e) => handleOnChange('total_csgt_records', e.target.value)} min="0" onKeyDown={handleKeyPress}/>
                    </Form.Item>
                    <Form.Item
                        ...
                        rules={[{ required: true, message: 'Vui lòng nhập số hồ sơ hệ lực lượng CSGT!' }]}
                    >
                        <InputComponent type="number" name="total_csgt_records_digitized" value={stateAdministrativeProceduresForm2.total_csgt_records_digitized} onChange={(e) => handleOnChange('total_csgt_records_digitized', e.target.value)} min="0" onKeyDown={handleKeyPress}/>
                    </Form.Item>
                    <Form.Item
                    ...
                    >
                        <InputComponent type="number" name="total_admin_records" value={stateAdministrativeProceduresForm2.total_admin_records} onChange={(e) => handleOnChange('total_admin_records', e.target.value)} min="0" onKeyDown={handleKeyPress}/>
                    </Form.Item>
                    <Form.Item
                        ...
                        rules={[{ required: true, message: 'Vui lòng nhập số hồ sơ hệ lực lượng Quản lý hành chính!' }]}
                    >
                        <InputComponent type="number" name="total_admin_records_digitized" value={stateAdministrativeProceduresForm2.total_admin_records_digitized} onChange={(e) => handleOnChange('total_admin_records_digitized', e.target.value)} min="0" onKeyDown={handleKeyPress}/>
                    </Form.Item>
                    <Form.Item
                    ...
                    >
                        <InputComponent type="number" name="total_firefighting_records" value={stateAdministrativeProceduresForm2.total_firefighting_records} onChange={(e) => handleOnChange('total_firefighting_records', e.target.value)} min="0" onKeyDown={handleKeyPress}/>
                    </Form.Item>
                    <Form.Item
                        ...
                        rules={[{ required: true, message: 'Vui lòng nhập số hồ sơ hệ lực lượng Phòng cháy chữa cháy!' }]}
                    >
                        <InputComponent type="number" name="total_firefighting_records_digitized" value={stateAdministrativeProceduresForm2.total_firefighting_records_digitized} onChange={(e) => handleOnChange('total_firefighting_records_digitized', e.target.value)} min="0" onKeyDown={handleKeyPress}/>
                    </Form.Item>
                    <Form.Item
                    ...
                    >
                        <InputComponent type="number" name="total_immigration_records" value={stateAdministrativeProceduresForm2.total_immigration_records} onChange={(e) => handleOnChange('total_immigration_records', e.target.value)} min="0" onKeyDown={handleKeyPress}/>
                    </Form.Item>
                    <Form.Item
                        ...
                        rules={[{ required: true, message: 'Vui lòng nhập số hồ sơ hệ lực lượng Xuất nhập cảnh!' }]}
                    >
                        <InputComponent type="number" name="total_immigration_records_digitized" value={stateAdministrativeProceduresForm2.total_immigration_records_digitized} onChange={(e) => handleOnChange('total_immigration_records_digitized', e.target.value)} min="0" onKeyDown={handleKeyPress}/>
                    </Form.Item>
